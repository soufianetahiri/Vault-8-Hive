...
// Modified by dan (motherbrain@hushmail.com), 27-Dec-2000, 
//   fixed bug with characters delayed until newline
    ...
    )
{
    ...
    i = WaitForMultipleObjects(3, HandleArray, FALSE, INFINITE);//0xffffffff);
    ...
    )
{
                    ...
                    &BytesRead, NULL, NULL)) 
    {
        ...
        if ( farm9crypt_write( Session->ClientSocket, Buffer, BytesRead ) < 0 )  
		{       
			break;
		} 
    ...
    )
{
    ...
    //
    while (( result = farm9crypt_read(Session->ClientSocket, Buffer, sizeof(Buffer))) > 0) {

        EchoCnt = 0;
		BufferCnt = strlen( Buffer );

		// Trap exit as it causes problems
		if (strnicmp(Buffer, "exit\r\n", 6) == 0)
			ExitThread(0);

//		printf("SessionWriteShellThreadFn: OK, we got a line in SessionWriteShellThreadFn\n" );
// SECURITY FIX: CW 12/27/04 Add BufferCnt size check.  If we hit end of buffer, flush it. Code taken from http://www.vulnwatch.org/netcat/netcat-111.txt.
        if (Buffer[0] == '\n' || Buffer[0] == '\r' || BufferCnt < BUFFER_SIZE-1) {
