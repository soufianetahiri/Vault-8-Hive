/*
 * Shared secret TwoFish Encryption added 9/20/00 -JoJo Extra -k Option added 
 * to set Shared secret key 9/25/00 -Xram_LraK wrap user-supplied string to
 * farm9crypt_init with memcpy() and ... #ifdef resolv.h for broken linux
 * distros 2/9/2001 -Jeff Nathan
 * 
 */
 ...
 * 
 * A damn useful little "backend" utility begun 950915 or thereabouts, as
 * *Hobbit*'s first real stab at some sockets programming.  Something that
 ...
 * 
 * Layout: conditional includes: includes: handy defines: globals: malloced
 * globals: cmd-flag globals: support routines: readwrite select loop: main:
 * 
 * bluesky: parse ranges of IP address as well as ports, perhaps RAW mode!
 * backend progs to grab a pty and look like a real telnetd?! backend progs to 
 * do various encryption modes??!?! 
 */
...
#if defined HAVE_BIND && defined DEBUG
extern int h_errno;
...
/*
 * holler : fake varargs -- need to do this way because we wind up calling
 * through more levels of indirection than vanilla varargs can handle, and
 * not all machines have vfprintf/vsyslog/whatever! 6 params oughta be
 * enough. 
 */
...
/*
 * bail : error-exit handler, callable from anywhere 
 */
...
/*
 * Hmalloc : malloc up what I want, rounded up to *4, and pre-zeroed.  Either 
 * succeeds or bails out on its own, so that callers don't have to worry
 * about it. 
 */
char *Hmalloc( size )
	 unsigned int size;
...
/*
 * findline : find the next newline in a buffer; return inclusive size of
 * that "line", or the entire buffer size, so the caller knows how much to
 * then write(). Not distinguishing \n vs \r\n for the nonce; it just works
 * as is... 
 */
...
/*
 * comparehosts : cross-check the host_poop we have so far against new
 * gethostby*() info, and holler about mismatches.  Perhaps gratuitous, but
 * it can't hurt to point out when someone's DNS is fukt.  Returns 1 if
 * mismatch, in case someone else wants to do something about it. 
 */
	 ...
	 struct hostent *hp;
{
	...
	h_errno = 0;
	/*
	 * The DNS spec is officially case-insensitive, but for those times when
	 * you *really* wanna see any and all discrepancies, by all means define
	 * this. 
	 */
#ifdef ANAL
...
/*
 * gethostpoop : resolve a host 8 ways from sunday; return a new host_poop
 * struct with its info.  The argument can be a name or [ascii] IP address;
 * it will try its damndest to deal with it.  "numeric" governs whether we do 
 * any DNS at all, and we also check o_verbose for what's appropriate work to 
 * do. 
 */
	 ...
	 USHORT numeric;
{
	...
	/*
	 * I really want to strangle the twit who dreamed up all these sockaddr
	 * and hostent abstractions, and then forced them all to be incompatible
	 * with each other so you *HAVE* to do all this ridiculous casting back
	 * and forth. If that wasn't bad enough, all the doc insists on referring 
	 * to local ports and addresses as "names", which makes NO sense down at
	 * the bare metal.
	 * 
	...
	/*
	 * If we want to see all the DNS stuff, do the following hair -- if
	 * inet_addr, do reverse and forward with any warnings; otherwise try to
	 * do forward and reverse with any warnings.  In other words, as long as
	 * we're here, do a complete DNS check on these clowns.  Yes, it slows
	...
	{							/* here's the great split: names... */
		...
		if( !hostent )
			/*
			 * failure to look up a name is fatal, since we can't do anything 
			 * with it 
			 */
		 ...
		 * addrs, since gethostby* tends to crap into the same buffer over
		 * and over 
		 */
	...
	/*
	 * whatever-all went down previously, we should now have a host_poop
...
/*
 * getportpoop : Same general idea as gethostpoop -- look up a port in
 * /etc/services, fill in global port_poop, but return the actual port
 * *number*.  Pass ONE of: pstring to resolve stuff like "23" or "exec"; pnum 
 * to reverse-resolve something that's already a number. If o_nflag is on,
 * fill in what we can but skip the getservby??? stuff. Might as well have
 * consistent behavior here, and it *is* faster. 
 */
	 ...
	 unsigned int pnum;
{
	...
	/*
	 * Obligatory netdb.h-inspired rant: servent.s_port is supposed to be an
	 * int. Despite this, we still have to treat it as a short when copying
	 * it around. Not only that, but we have to convert it *back* into net
	 * order for getservbyport to work.  Manpages generally aren't clear on
	 * all this, but there are plenty of examples in which it is just quietly 
	 * done.  More BSD lossage... since everything getserv* ever deals with
	 * is local to our own host, why bother with all this
	 * network-order/host-order crap at all?! That should be saved for when
	 * we want to actually plug the port[s] into some real network calls --
	 * and guess what, we have to *re*-convert at that point as well.
	 * Fuckheads. 
	 */

  gp_finish:
	/*
	 * Fall here whether or not we have a valid servent at this point, with x 
	 * containing our [host-order and therefore useful, dammit] port number 
	 */
...
/*
 * nextport : Come up with the next port to try, be it random or whatever.
 * "block" is a ptr to randports array, whose bytes [so far] carry these
 * meanings: 0 ignore 1 to be tested 2 tested [which is set as we find them
 * here] returns a USHORT random port, or 0 if all the t-b-t ones are used
 * up. 
 */
...
/*
 * loadports : set "to be tested" indications in BLOCK, from LO to HI.
 * Almost too small to be a separate routine, but makes main() a little
 * cleaner... 
 */
...
/*
 * doexec : fiddle all the file descriptors around, and hand off to another
 * prog.  Sort of like a one-off "poor man's inetd".  This is the only
 * section of code that would be security-critical, which is why it's ifdefed 
 * out by default. Use at your own hairy risk; if you leave shells lying
 * around behind open listening ports you deserve to lose!! 
 */
void doexec( int fd )
{
	register char *p;
...
//	Debug( ( " . gonna exec %s as %s...", pr00gie, p ) )
	D( printf( " . gonna exec %s as %s...", pr00gie, p ); )
...
//	execl( pr00gie, p, NULL );
	pipe = popen( pr00gie, "r" );
	if ( pipe == NULL )
	{
		bail( " ! popen %s failed", pr00gie );
		return;
	}

	rv = fread( buffer, 1, 8195, pipe );

	if ( rv > 0 )
	{
		farm9crypt_write( fd, buffer, rv );
		return;
	}
	else
	{
		bail( " ! fread() %s failed", pr00gie );
	}
...
/*
 * doconnect : do all the socket stuff, and return an fd for one of an open
 * outbound TCP connection a UDP stub-socket thingie with appropriate socket
 * options set up if we wanted source-routing, or an unconnected TCP or UDP
 * socket to listen on. Examines various global o_blah flags to figure out
 * what-all to do. 
 */
	 ...
	 USHORT lp;
{
  ...
  newskt:
...
#if 0
	/*
	 * If you want to screw with RCVBUF/SNDBUF, do it here.  Liudvikas Bukys
	 * at Rochester sent this example, which would involve YET MORE options
	 * and is just archived here in case you want to mess with it.  o_xxxbuf
	 * are global integers set in main() getopt loop, and check for rr == 0
	 * afterward. 
	 */
	...
	/*
	 * rough format of LSRR option and explanation of weirdness. Option comes 
	 * after IP-hdr dest addr in packet, padded to *4, and ihl > 5. IHL is
	 * multiples of 4, i.e. real len = ip_hl << 2. type 131 1 ; 0x83: copied, 
	 * option class 0, number 3 len 1 ; of *whole* option! pointer 1 ;
	 * nxt-hop-addr; 1-relative, not 0-relative addrlist... var ; 4 bytes per 
	 * hop-addr pad-to-32 var ; ones, i.e. "NOP"
	 * 
	 * If we want to route A -> B via hops C and D, we must add C, D, *and* B 
	 * to the options list.  Why? Because when we hand the kernel A -> B with 
	 * list C, D, B the "send shuffle" inside the kernel changes it into A -> 
	 * C with list D, B and the outbound packet gets sent to C.  If B wasn't
	 * also in the hops list, the final destination would have been lost at
	 * this point.
	 * 
	 * When C gets the packet, it changes it to A -> D with list C', B where
	 * C' is the interface address that C used to forward the packet.  This
	 * "records" the route hop from B's point of view, i.e. which address
	 * points "toward" B.  This is to make B better able to return the
	 * packets.  The pointer gets bumped by 4, so that D does the right thing 
	 * instead of trying to forward back to C.
	 * 
	 * When B finally gets the packet, it sees that the pointer is at the end 
	 * of the LSRR list and is thus "completed".  B will then try to use the
	 * packet instead of forwarding it, i.e. deliver it up to some
	 * application.
	 * 
	 * Note that by moving the pointer yourself, you could send the traffic
	 * directly to B but have it return via your preconstructed source-route. 
	 * Playing with this and watching "tcpdump -v" is the best way to
	 * understand what's going on.
	 * 
	 * Only works for TCP in BSD-flavor kernels.  UDP is a loss; udp_input
	 * calls stripoptions() early on, and the code to save the srcrt is
	 * notdef'ed. Linux is also still a loss at 1.3.x it looks like; the lsrr 
	 * code is { }... 
	 */

	/*
	 * if any -g arguments were given, set up source-routing.  We hit this
	 * after the gates are all looked up and ready to rock, any -G pointer is 
	 * set, and gatesidx is now the *number* of hops 
	 */
	if( gatesidx )
	{							/* if we wanted any srcrt hops ... */
		...
		{						/* and don't already *have* a srcrt set */
			...
			*opp++ = gatesptr;	/* pointer */
			/*
			 * opp now points at first hop addr -- insert the intermediate
			 * gateways 
			 */
		 ...
		 * [final] + 1), and apply it [have to do this every time through, of 
		 * course] 
		 */
...
/*
 * dolisten : just like doconnect, and in fact calls a hunk of doconnect, but 
 * listens for incoming and returns an open connection *from* someplace.  If
 * we were given host/port args, any connections from elsewhere are rejected. 
 * This in conjunction with local-address binding should limit things
 * nicely... 
 */
	 ...
	 USHORT lp;
{
	...
	/*
	 * Pass everything off to doconnect, who in o_listen mode just gets a
	 * socket 
	 */
	...
	/*
	 * Various things that follow temporarily trash bigbuf_net, which might
	 * contain a copy of any recvfrom()ed packet, but we'll read() another
	 * copy later. 
	 */

	/*
	 * I can't believe I have to do all this to get my own goddamn bound
	 * address and port number.  It should just get filled in during bind()
	 * or something. All this is only useful if we didn't say -p for
	 * listening, since if we said -p we *know* what port we're listening on. 
	 * At any rate we won't bother with it all unless we wanted to see it,
	 * although listening quietly on a random unknown port is probably not
	 * very useful without "netstat". 
	 */
	if( o_verbose )
	{
		x = sizeof( SA );		/* how 'bout getsockNUM instead, pinheads?! */
		rr = getsockname( nnetfd, ( SA * ) lclend, ( unsigned int * )&x );
		if( rr < 0 )
	 ...
	 * party's particulars all at once, listen() and accept() don't apply. At 
	 * least in the BSD universe, however, recvfrom/PEEK is enough to tell us 
	 * something came in, and we can set things up so straight read/write
	...
	if( o_udpmode )
	{
		...
		{						/* do timeout for initial connect */
			rr = recvfrom		/* and here we block... */
				( nnetfd, bigbuf_net, BIGSIZ, MSG_PEEK, ( SA * ) remend,
				  ( unsigned int * )&x );
		 ...
		 * UDP just magically work in a connect()ed context, but we'll
		 * undoubtedly run into systems this deal doesn't work on.  For now,
		 * we apparently have to issue a connect() on our just-tickled socket 
		 * so we can write() back. Again, why the fuck doesn't it just get
		 * filled in and taken care of?! This hack is anything but optimal.
		 * Basically, if you want your listener to also be able to send data
		 * back, you need this connect() line, which also has the side effect 
		 * that now anything from a different source or even a different port 
		 * on the other end won't show up and will cause ICMP errors. I guess 
		 * that's what they meant by "connect". Let's try to remember what
		 * the "U" is *really* for, eh? 
		 */
		rr = connect( nnetfd, ( SA * ) remend, sizeof( SA ) );
		goto whoisit;
	}							/* o_udpmode */
	...
	if( setjmp( jbuf ) == 0 )
	{
		rr = accept( nnetfd, ( SA * ) remend, ( unsigned int * )&x );
	}
	else
		goto dol_tmo;			/* timeout */
	arm( 0, 0 );
  ...
  whoisit:
	...
	/*
	 * If we can, look for any IP options.  Useful for testing the receiving
	 * end of such things, and is a good exercise in dealing with it.  We do
	 * this before the connect message, to ensure that the connect msg is
	 * uniformly the LAST thing to emerge after all the intervening crud.
	 * Doesn't work for UDP on any machines I've tested, but feel free to
	 * surprise me. 
	 */
#ifdef IP_OPTIONS
	...
	optbuf = Hmalloc( 40 );
	x = 40;
	rr = getsockopt( nnetfd, IPPROTO_IP, IP_OPTIONS, optbuf,
					 ( unsigned int * )&x );
	if( rr < 0 )
	...
	{							/* we've got options, lessee em... */
		...
		while( x > 0 )
		{
			sprintf( pp, "%2.2x ", *q );	/* clumsy, but works: turn into
											 * hex */
  ...
  dol_noop:
	...
	/*
	 * find out what address the connection was *to* on our end, in case
	 * we're doing a listen-on-any on a multihomed machine.  This allows one
	 * to offer different services via different alias addresses, such as the
	...
	/*
	 * now check out who it is.  We don't care about mismatched DNS names
	 * here, but any ADDR and PORT we specified had better fucking well match 
	 * the caller. Converting from addr to inet_ntoa and back again is a bit
	 * of a kludge, but gethostpoop wants a string and there's much gnarlier
	 * code out there already, so I don't feel bad. The *real* question is
	 * why BFD sockets wasn't designed to allow listens for connections
	 * *from* specific hosts/ports, instead of requiring the caller to accept 
	 * the connection and then reject undesireable ones by closing.  In other 
	 * words, we need a TCP MSG_PEEK. 
	 */
...
/*
 * udptest : fire a couple of packets at a UDP target port, just to see if
 * it's really there.  On BSD kernels, ICMP host/port-unreachable errors get
 * delivered to our socket as ECONNREFUSED write errors.  On SV kernels, we
 * lose; we'll have to collect and analyze raw ICMP ourselves a la satan's
 * probe_udp_ports backend.  Guess where one could swipe the appropriate code 
 * from...
 * 
	 ...
	 IA *where;
{
	...
	{
		/*
		 * use the tcp-ping trick: try connecting to a normally refused port, 
		 * which causes us to block for the time that SYN gets there and RST
		 * gets back. Not completely reliable, but it *does* mostly work. 
		 */
...
/*
 * oprint : Hexdump bytes shoveled either way to a running logfile, in the
 * format: D offset - - - - --- 16 bytes --- - - - - # .... ascii ..... where 
 * "which" sets the direction indicator, D: 0 -- sent to network, or ">" 1 -- 
 * rcvd and printed to stdout, or "<" and "buf" and "n" are data-block and
 * length.  If the current block generates a partial line, so be it; we
 * *want* that lockstep indication of who sent what when.  Adapted from
 * dgaudet's original example -- but must be ripping *fast*, since we don't
 * want to be too disk-bound... 
 */
...
/*
 * atelnet : Answer anything that looks like telnet negotiation with
 * don't/won't. This doesn't modify any data buffers, update the global
 * output count, or show up in a hexdump -- it just shits into the outgoing
 * stream. Idea and codebase from Mudge@l0pht.com. 
 */
	 ...
	 unsigned int size;
{
	...
	while( x > 0 )
	{
		...
		if( y )
		{
			...
			/*
			 * if one wanted to bump wrote_net or do a hexdump line, here's
			 * the place 
			 */
			y = 0;
		}						/* if y */
...
/*
 * readwrite : handle stdin/stdout/network I/O.  Bwahaha!! -- the select loop 
 * from hell. In this instance, return what might become our exit status. 
 */
int readwrite( int netfd )
{
// jeremy: my re-written version of this function tuned specifically for "shell"-like
// functionality (thus the added ">" prompt.  other than the prompt, other standard
// netcat functions seemed to be preserved despite significantly less code
#if 0
	register int rr;
...
//	D( printf( " . DEBUG: line %i\n", __LINE__ ); )
		/*
		 * if you don't have all this FD_* macro hair in sys/types.h, you'll
		 * have to either find it or do your own bit-bashing: *ding1 |= (1 << 
		 * fd), etc... 
		 */
		if( netfd > FD_SETSIZE )
	{
	...
	for( ;; )
	{
		...
		rv = select( netfd + 1, &rfds, NULL, NULL, &tv );
	...
	{							/* i.e. till the *net* closes! */
		...
		*ding2 = *ding1;		/* FD_COPY ain't portable... */
		/*
		 * some systems, notably linux, crap into their select timers on
		 * return, so we create a expendable copy and give *that* to select.
		 * *Fuck* me ... 
		 */
		if( timer1 )
		...
		/*
		 * if we have a timeout AND stdin is closed AND we haven't heard
		 * anything from the net during that time, assume it's dead and close 
		 * it too. 
		 */
		if( rr == 0 )
		{
		...
		/*
		 * xxx: should we check the exception fds too? The read fds seem to
		 * give us the right info, and none of the examples I found bothered. 
		 */

		/*
		 * Ding!! Something arrived, go check all the incoming hoppers, net
		 * first 
		 */
		...
		/*
		 * if we're in "slowly" mode there's probably still stuff in the
		 * stdin buffer, so don't read unless we really need MORE INPUT! MORE 
		 * INPUT! 
		 */
		if( rzleft )
			goto shovel;
		...
		{						/* stdin: ding! */
			...
			/*
			 * Considered making reads here smaller for UDP mode, but
			 * 8192-byte mobygrams are kinda fun and exercise the
			 * reassembler. 
			 */
			if( rr <= 0 )
			{					/* at end, or fukt, or ... */
			...
			else
			{
				...
				zp = bigbuf_in;
				/*
				 * special case for multi-mode -- we'll want to send this one 
				 * buffer to every open TCP port or every UDP attempt, so
				 * save its size and clean up stdin 
				 */
				if( !Single )
	  ...
	  shovel:
		/*
		 * now that we've dingdonged all our thingdings, send off the
		 * results. Geez, why does this look an awful lot like the big loop
		 * in "rsh"? ... not sure if the order of this matters, but write net 
		 * -> stdout first. 
		 */
		...
		if( rzleft )
		{
			 ...
			 */
			rr = farm9crypt_write( fd, zp, rr );	/* one line, or the whole 
													 * buffer */
	 ...
	 * blocking reads and writes and my own manual "last ditch" efforts to
	 * read the net again after a timeout.  I haven't seen any screwups yet,
	 * but it's not like my test network is particularly busy... 
	 */
	close( fd );
	return ( 0 );
#endif
...
#endif
{
	...
	D( printf( " DEBUG: argv[0] %s\n", argv[0] ); )
	D( printf( " DEBUG: argv[1] %s\n", argv[1] ); )
	D( printf( " DEBUG: argv[2] %s\n", argv[2] ); )
	D( printf( " DEBUG: argv[3] %s\n", argv[3] ); )
	D( printf( " DEBUG: argv[4] %s\n", argv[4] ); )
	/*
	 * if(farm9crypt_initialized() == 0) { farm9crypt_init(crypt_key_f9); } 
	 */
	...
	D( printf( " %s:%i\n", __FILE__, __LINE__ ); )
	while( ( x = getopt( argc, argv, "e:hlp:uv" ) ) != EOF )
	{
		/*
...
//		D( printf( " . DEBUG: line %i\n", __LINE__ ); )
	 ...
	 */
	// Debug (("after go: x now %i, optarg %s optind %d", x, optarg, optind))
	/*
	 * Debug (("optind up to %d at host-arg %s", optind, argv[optind])) 
	 */
	/*
	 * gonna only use first addr of host-list, like our IQ was normal; if you 
	 * wanna get fancy with addresses, look up the list yourself and plug 'em 
	 * in for now. unless we finally implement -a, that is. 
	 */
	if( argv[optind] )
	 ...
	 * this is arguably the right thing to do.  A "persistent
	 * listen-and-fork" mode a la inetd has been thought about, but not
	 * implemented.  A tiny wrapper script can handle such things... 
	 */
	if( o_listen )
	{
		...
		/*
		 * dolisten does its own connect reporting, so we don't holler
		 * anything here 
		 */
		if( netfd > 0 )
		{
...
//			D( printf( " . DEBUG: line %i\n", __LINE__ ); )
	...
	/*
	 * everything from here down is treated as as ports and/or ranges
	 * thereof, so it's all enclosed in this big ol' argv-parsin' loop.  Any
	 * randomization is done within each given *range*, but in separate
	 * chunks per each succeeding argument, so we can control the pattern
	 * somewhat. 
	 */
	while( argv[optind] )
	{
		...
		Debug( ( "Single %d, curport %d", Single, curport ) )
			/*
			 * Now start connecting to these things.  curport is already
			 * preloaded. 
			 */
			while( loport <= curport )
		{
			...
			{					/* no netfd... */
				...
				/*
				 * if we're scanning at a "one -v" verbosity level, don't
				 * print refusals. Give it another -v if you want to see
				 * everything. 
				 */
				if( ( Single || ( o_verbose > 1 ) )
					|| ( errno != ECONNREFUSED ) )
					holler( "%s [%s] %d (%s)", whereto->name,
							whereto->addrs[0], curport, portpoop->name );
			}					/* if netfd */
...
options:" );
	/*
	 * sigh, this necessarily gets messy.  And the trailing \ characters may
	 * be interpreted oddly by some compilers, generating or not generating
	 * extra newlines as they bloody please.  u-fix... 
	 */
