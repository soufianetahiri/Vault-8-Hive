...
//    s1 = (unsigned char *) strstr( (char *) buf, "-----BEGIN CERTIFICATE-----" );
    s1 = (unsigned char *) strstr( (char *) buf, (const char *) pol_bc );

    if( s1 != NULL )
    {
//	s2 = (unsigned char *) strstr( (char *) buf, "-----END CERTIFICATE-----" );
	s2 = (unsigned char *) strstr( (char *) buf, (const char *) pol_ec );
    ...
    if( crt->version == 3 )
    {
        ret = x509_get_crt_ext( &p, end, &crt->v3_ext, &crt->ca_istrue, &crt->max_pathlen );
        if( ret != 0 )
        {
            x509_free( crt );
            return( ret );
    ...
    {
        x509_free( crt );
        return( POLARSSL_ERR_X509_CERT_INVALID_FORMAT | POLARSSL_ERR_ASN1_LENGTH_MISMATCH );
    }
...
//    s1 = (unsigned char *) strstr( (char *) buf, "-----BEGIN X509 CRL-----" );
    s1 = (unsigned char *) strstr( (char *) buf, (const char *) pol_bxcrl );

    if( s1 != NULL )
    {
//        s2 = (unsigned char *) strstr( (char *) buf, "-----END X509 CRL-----" );
        s2 = (unsigned char *) strstr( (char *) buf, (const char *) pol_excrl );
...
//    s1 = (unsigned char *) strstr( (char *) key, "-----BEGIN RSA PRIVATE KEY-----" );
    s1 = (unsigned char *) strstr( (char *) key, (const char *) pol_brpk );

    if( s1 != NULL )
    {
//        s2 = (unsigned char *) strstr( (char *) key, "-----END RSA PRIVATE KEY-----" );
        s2 = (unsigned char *) strstr( (char *) key, (const char *) pol_erpk );
...
int x509parse_dn_gets( char *buf, size_t size, const x509_name *dn )
{
    ...
    while( name != NULL )
    {
        ...
        if( memcmp( name->oid.p, OID_X520, 2 ) == 0 )
        {
            ...
            default:
                ret = snprintf( p, n, "0x%02X=", name->oid.p[2] );
                break;
            }
        SAFE_SNPRINTF();
        }
        else if( memcmp( name->oid.p, OID_PKCS9, 8 ) == 0 )
        {
            ...
            {
            case PKCS9_EMAIL:
//                ret = snprintf( p, n, "emailAddress=" );
                ret = snprintf( p, n, (const char *) pol_ema);
                break;

            default:
                ret = snprintf( p, n, "0x%02X=", name->oid.p[8] );
                break;
            }
        SAFE_SNPRINTF();
        }
...
//    ret = snprintf( p, n, "%scert. version : %d\n", prefix, crt->version );
    ret = snprintf( p, n, (const char *) pol_cv, prefix, crt->version );
    SAFE_SNPRINTF();
    //    ret = snprintf( p, n, "%sserial number : ", prefix );
    ret = snprintf( p, n, (const char *) pol_sn, prefix );
    ...
    for( i = 0; i < nr; i++ )
    {
        ret = snprintf( p, n, "%02X%s", crt->serial.p[i], ( i < nr - 1 ) ? ":" : "" );
        SAFE_SNPRINTF();
    }

//    ret = snprintf( p, n, "\n%sissuer name   : ", prefix );
    ret = snprintf( p, n, (const char *) pol_in, prefix );
    SAFE_SNPRINTF();
    ret = x509parse_dn_gets( p, n, &crt->issuer  );
    SAFE_SNPRINTF();

//    ret = snprintf( p, n, "\n%ssubject name  : ", prefix );
    ret = snprintf( p, n, (const char *) pol_subn, prefix );
    SAFE_SNPRINTF();
    ret = x509parse_dn_gets( p, n, &crt->subject );
    SAFE_SNPRINTF();

//    ret = snprintf( p, n, "\n%sissued  on    : %04d-%02d-%02d %02d:%02d:%02d"
    ret = snprintf( p, n, (const char *) pol_io, prefix,
...
//    ret = snprintf( p, n, "\n%sexpires on    : %04d-%02d-%02d %02d:%02d:%02d", prefix,
    ret = snprintf( p, n, (const char *) pol_expo, prefix,
...
//    ret = snprintf( p, n, "\n%ssigned using  : RSA+", prefix );
    ret = snprintf( p, n, (const char *) pol_susr, prefix );
    SAFE_SNPRINTF();
...
//    case SIG_RSA_SHA512 : ret = snprintf( p, n, "SHA512" ); break;
        case SIG_RSA_MD2    : ret = snprintf( p, n, (const char *) pol_h1   ); break;
        case SIG_RSA_MD4    : ret = snprintf( p, n, (const char *) pol_h2   ); break;
        case SIG_RSA_MD5    : ret = snprintf( p, n, (const char *) pol_h3   ); break;
        case SIG_RSA_SHA1   : ret = snprintf( p, n, (const char *) pol_h4   ); break;
        case SIG_RSA_SHA224 : ret = snprintf( p, n, (const char *) pol_h5   ); break;
        case SIG_RSA_SHA256 : ret = snprintf( p, n, (const char *) pol_h6   ); break;
        case SIG_RSA_SHA384 : ret = snprintf( p, n, (const char *) pol_h7   ); break;
        case SIG_RSA_SHA512 : ret = snprintf( p, n, (const char *) pol_h8   ); break;
        default: ret = snprintf( p, n, "???"  ); break;
    }
    SAFE_SNPRINTF();

//    ret = snprintf( p, n, "\n%sRSA key size  : %d bits\n", prefix, crt->rsa.N.n * (int) sizeof( unsigned long ) * 8 );
    ret = snprintf( p, n, (const char *) pol_rks, prefix, crt->rsa.N.n * (int) sizeof( unsigned long ) * 8 );
    SAFE_SNPRINTF();
...
//    ret = snprintf( p, n, "%sCRL version   : %d", prefix, crl->version );
    ret = snprintf( p, n, (const char *) pol_crlv, prefix, crl->version );
    SAFE_SNPRINTF();
...
//    ret = snprintf( p, n, "\n%sRevoked certificates:", prefix );
    ret = snprintf( p, n, (const char *) pol_rc, prefix );
    SAFE_SNPRINTF();
...
//        ret = snprintf( p, n, "\n%sserial number: ", prefix );
        ...
        for( i = 0; i < nr; i++ ) {
            ret = snprintf( p, n, "%02X%s", entry->serial.p[i], ( i < nr - 1 ) ? ":" : "" );
            SAFE_SNPRINTF();
        }
        
//        ret = snprintf( p, n, " revocation date: "  "%04d-%02d-%02d %02d:%02d:%02d",
        ret = snprintf( p, n, (const char *) pol_rd,
